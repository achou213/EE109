...
{
    PORTB &=~(1 << 0);/* Clear PB0 to 0 for a command transfer */
...
{
    PORTB |= (1 << 0); /* Set PB0 to 1 for a data transfer */

    lcd_writenibble(dat);/* Call lcd_writenibble to send UPPER four bits of "dat" argument */

    lcd_writenibble(dat << 4); /* Call lcd_writenibble to send LOWER four bits of "dat" argument */
...
void lcd_writenibble(unsigned char lcdbits)
{
	...
	PORTD &= ~(DATA_BITS); //clears bits in destination register
	PORTD |= (lcdbits & DATA_BITS); 

    PORTB |= (1 << 1); /* Make E signal (PB1) go to 1 and back to 0 */
